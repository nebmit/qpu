<script lang="ts">
    ...
    let connEdges = $derived(
        edges.filter(
            (e: UiEdge) => e.source === qubitId || e.target === qubitId,
        ),
    );
...
>
    <div class="flex justify-between items-start mb-[14px]">
        <div>
            <div
                class="font-mono text-[22px] font-normal text-(--text) leading-none"
            >
    ...
    <div
        class="flex justify-between items-baseline py-1.75 border-b border-black/5"
    >
    ...
    >
        ...
        <span class="font-mono text-[13px] text-(--text)"
            >{formatPercent(qubit.readout_error)}</span
        >
    </div>
    <div
        class="flex justify-between items-baseline py-1.75 border-b border-black/5"
    ...
    >
        ...
        <span class="font-mono text-[13px] text-(--text)"
            >{formatPercent(qubit.p10)}</span
        >
    </div>
    {#if connEdges.length > 0}
        <div
            class="mt-3.25 mb-1.75 text-[11px] text-(--text-3) tracking-[0.06em] uppercase font-medium"
        >
            CX Gate Errors
        </div>
        {#each connEdges as e (e.source + '-' + e.target)}
            ...
            <div
                class="flex justify-between items-baseline py-1.25 border-b border-black/5"
            >
                <span class="text-[12px] text-(--text-3) font-mono"
                    >↔ Q{nb}</span
                >
                <span class="font-mono text-[13px] text-(--text)"
                    >{e.cx_error == null
